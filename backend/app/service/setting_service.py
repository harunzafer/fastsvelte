...
class SettingService:
    ...
    def _validate_value(self, value: str, setting_type: SettingType) -> None:
        try:
            ...
            raise ValueError(f"Invalid value for type {setting_type}: {e}")

    async def set_user_setting_by_key(self, user_id: int, key: str, value: str) -> UserSetting:
        ...
        return await self.user_setting_repo.set_user_setting(user_id, definition.id, value)

    async def set_organization_setting_by_key(self, org_id: int, key: str, value: str) -> OrganizationSetting:
        definition = await self.setting_repo.get_org_setting_definition(key)
        if not definition:
            raise ValueError(f"Unknown organization setting: {key}")
        self._validate_value(value, definition.type)
        return await self.organization_setting_repo.set_organization_setting(org_id, definition.id, value)
    ...
    async def list_organization_settings(self, organization_id: int) -> list[OrganizationSetting]:
        return await self.organization_setting_repo.list_organization_settings(organization_id)
