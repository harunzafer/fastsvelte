...
async def get_user_settings(
    ...
    if user_id != current_user.id:
        ...
        if current_user.role != Role.SYSTEM_ADMIN and (
            current_user.role != Role.ORG_ADMIN
            or target_user.organization_id != current_user.organization_id
        ):
            raise HTTPException(
                status_code=403, detail="Not authorized to view settings for this user"
            )
...
@router.get(
    "/organization/{organization_id}", operation_id="getOrganizationSettings"
)
@inject
async def get_org_settings(
    ...
    if (
        ...
        raise HTTPException(
            status_code=403,
            detail="Not authorized to view settings for this organization",
        )
    return await setting_service.list_organization_settings(organization_id)


@router.post(
    "/organization/{organization_id}", operation_id="setOrganizationSetting"
)
@inject
