...
export function getOAuthErrorMessage(errorCode: string): string {
	return (
		OAUTH_ERROR_MESSAGES[errorCode] || 'An error occurred during Google login. Please try again.'
	);
}
 ...
 */
export function createGoogleOAuthHandler(apiError: { value: string }, loading: { value: boolean }) {
	return () =>
		initiateGoogleOAuth(
